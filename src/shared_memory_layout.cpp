...
{
    if (agents.empty()) {
        ERR_PRINT("SharedMemoryLayout: agents list is empty.");
        return false;
    }
...
void SharedMemoryLayout::set_agents(const std::vector<HPAAgentNode *> &agents) {
    d_agents = agents;
}

size_t SharedMemoryLayout::total_size() const {
...
void SharedMemoryLayout::write_env_state(void *shm) const {
...
void SharedMemoryLayout::dispatch_actions(const void *shm) const {
...
uint8_t *SharedMemoryLayout::visual_obs_block_ptr(void *shm) const {
    return static_cast<uint8_t *>(shm) + _visual_obs_offset();
}

// --- Private offset helpers ---

size_t SharedMemoryLayout::_visual_obs_offset() const {
    return sizeof(Header);
}

size_t SharedMemoryLayout::_scalar_obs_offset() const {
    return _visual_obs_offset() + d_num_envs * d_visual_obs_size;
}

size_t SharedMemoryLayout::_rewards_offset() const {
    return _scalar_obs_offset() + d_num_envs * d_scalar_obs_size;
}

size_t SharedMemoryLayout::_done_flags_offset() const {
    return _rewards_offset() + d_num_envs * sizeof(float);
}

size_t SharedMemoryLayout::_actions_offset() const {

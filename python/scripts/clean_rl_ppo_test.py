...
class Args:
    ...
    """save a checkpoint every this many iterations (0 to disable)"""
    resume_from: str | None = None
    """path to a .cleanrl_model checkpoint to resume weights from"""

    # Godot environment arguments
    project_path: str = ""
    """path to the Godot project directory"""
    godot_binary: str | None = None
    """path to the Godot binary; if None, Godot must be started manually"""
    num_instances: int = 1
    """number of parallel Godot instances"""
    obs_width: int = 64
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        # Derive the flat encoder output size for whatever obs resolution is used
        with torch.no_grad():
            dummy = torch.zeros(1, n_channels, obs_shape[0], obs_shape[1])
            enc_out = self.encoder(dummy).shape[1]
        self.fc = nn.Sequential(
            layer_init(nn.Linear(enc_out, 512)),
            nn.ReLU(),
        )
...
if __name__ == "__main__":
    ...
    run_name = f"roomba__{args.exp_name}__{args.seed}__{int(time.time())}"
    if args.track:
        import wandb
    ...
    if args.resume_from is not None:
        agent.load_state_dict(torch.load(args.resume_from, map_location=device, weights_only=True))
        print(f"resumed weights from {args.resume_from}")
    optimizer = optim.Adam(agent.parameters(), lr=args.learning_rate, eps=1e-5)

    # ALGO Logic: Storage setup
    # Stored as uint8 on CPU (16× smaller than float32 on GPU); moved to device per minibatch in the update loop.
    obs = torch.zeros((args.num_steps, args.num_envs) + envs.single_observation_space.shape, dtype=torch.uint8)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogprob, entropy, newvalue = agent.get_action_and_value(b_obs[mb_inds].to(device), b_actions[mb_inds])
        ...
        writer.add_scalar("charts/mean_reward_per_step", rewards.mean().item(), global_step)
        ...
        if args.save_model and args.checkpoint_interval > 0 and iteration % args.checkpoint_interval == 0:
